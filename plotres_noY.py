...
def plotres_noY(res, priors=[0,0,0,0], startn=0, color1='blue', color2='gray', color3='red', ls1=':', lsty='-', lw1=1.5, ms1=0.15, mk='ko'):
    """
    ...
    """
    ############  set up figure  ############
    fig = matplotlib.pylab.figure(figsize=(13, 10))    # New figure
    ...
    varnames=['log(age)','[Fe/H]','Dist Mod','A$_{V}$','Iteration']
    xx=arange(0,len(age))
    for i,axes in enumerate([ax,bx,cx,dx]):
    ...
    varnames2=['log(age)  ','[Fe/H]    ','Dist Mod  ','A$_{V}$   ']
    ...
    CIs=[age_p,feh_p,dist_p,Av_p]
    for i,axes in enumerate([ax2,bx2,cx2,dx2]):
    ...
    dx2.set_xlabel('PDF')
    for axis in (ax2,bx2,cx2,dx2):
        axis.yaxis.tick_right()
    for axis in (ax,bx,cx,ax2,bx2,cx2,dx2):
        setp(axis.get_xticklabels(),visible=False)
    for axis in (ax,bx,cx,dx,ax2,bx2,cx2,dx2):
    ...
    axess=[ax,ax2,bx,bx2,cx,cx2,dx,dx2];j=0
